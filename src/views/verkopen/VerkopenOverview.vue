<template>
  <div class="content-body">
    <SearchBar :search.sync="params.search" :limit.sync="params.limit" @searchTable="searchTable" navigateRouteName="VerkopenCreateview" />
    <div class="box">
      <b-table
        @dblclick="rowClicked"
        narrowed
...
export default {
  ...
  created(){
    this.tableController = VerkopenController
    this.navigateRoute = Navigation.VK_UPDATE
    this.socketName = Socket.VERKOPEN
  },
  computed: {
    getWidth() {
      return TableColumns.getVerkoopColumns(this.updatedByIsVisible)
    }
  },
  methods: {
    changePaid(value, id){
      VerkopenController.paid(this, { id: id, isBetaald: value }, this.socket)
    },
  },
};
</script>